...
//     which has four text fields, and it does NOT include the
//     primary key "idx_{tableName}".
...
type TableDetails struct {
	...
	// (this is IMPORTANT TO NOTE!) omits the primary key
	// (which can be brought in when needed).
...
//    key (noted as "PKEY", and always named "idx_{tableName}"), for
//    (e.g.) output from SELECT
//  - a version (suffixed with "_noID") that does NOT include the 
//    primary key "PKEY", for (e.g.) input to INSERT (where the ID 
//    is new) and input to UPDATE (where the ID is used in a WHERE 
//    clause to find the record) ... altho the ID WHERE is optional 
//    if we decide to allow using the ID in the record passed in)
//
// NOTE: The inline docu for fields in this struct assume a 
// table that has the primary key plus four (4) other fields:
//  - Fields are numbered [0..3], i.e. as slice indices
//  - Value placeholders are wrtten Postgres style, "$1".."$4"c
// .
type ColumnStringsCSV struct {

	// FieldNames_* [with|no ID primary key] 
	// list column (i.e. field) names, in order:
	// (w/ ID) "PKEY, F0, F1, F2, F3"
	// (no ID)       "F0, F1, F2, F3" 
